...
export const getTrends = createAsyncThunk(
  ...
  async (woeid = 1, { dispatch }) => {
    let { locations, trends } = await request(`/api/trends?woeid=${woeid}`, {
      dispatch,
    });
...
const trendsSlice = createSlice({
  ...
  reducers: {},
  extraReducers: {
    [getTrends.rejected]: (state) => {
      state.status = "error";
    },
    [getTrends.pending]: (state) => {
...
export const trendsSelectors = trendsAdapter.getSelectors(
  (state) => state.trends
);
