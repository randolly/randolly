...
export default (props) => {
  ...
  let posts = useSelector((state) =>
    selectUserPosts(state, user && user.screen_name)
  );
  ...
  let userPosts = (
    <>
      <PostsList status={status} getPosts={getPosts} posts={posts} />
    </>
  );
      ...
      <>
        ...
        <div className="p-3 border-bottom">
          <Row className="d-flex justify-content-between mt-n2 px-2 align-items-center w-100">
            ...
            {authUser && authUser.screen_name === user.screen_name ? (
              ...
              <FollowButton
                user={user}
                followUser={() => {
                  dispatch(followUser(user.screen_name));
                }}
                unFollowUser={() => {
                  dispatch(unFollowUser(user.screen_name));
                }}
              />
            )}
          </Row>
          ...
          <Row className="d-flex my-2">
            ...
            >
              {numFormatter(user.followers_count)} <span>Followers</span>
            </Link>
            <Link
              to={`/user/${user.screen_name}/friends`}
              className="text-muted mr-2"
            >
              {numFormatter(user.friends_count)} <span>Following</span>
            </Link>
          </Row>
        </div>
