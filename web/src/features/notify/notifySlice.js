...
export const fetchNotifs = () => async (dispatch, getState) => {
  ...
  } else if (notifsInterval) clearInterval(notifsInterval);
  const {
    notify: { status },
  } = getState();
...
export const readNotif = createAsyncThunk(
  ...
  async (notifications, { dispatch }) => {
    dispatch(notifRead(notifications));
    return request(`/api/notification_read/`, { dispatch });
  }
);
// export const readNotif = createAsyncThunk(
//     'notifs/readNotif',
...
export const notifySelectors = notifyAdapter.getSelectors(
  (state) => state.notify
);
