...
export default ({ post, no_reply_tag = false }) => {
  ...
  let name1 =
    authUser && authUser.screen_name === post.user.screen_name
      ? "You"
      : "@" + post.user.screen_name;
  let name2 =
    authUser && authUser.screen_name === post.in_reply_to_screen_name
      ? "you"
  ...
  return (
    <>
      {retweeted_by && (no_reply_tag = true) && (
        ...
        >
          <small>@{retweeted_by.screen_name} reposted</small>
        </UserLink>
      )}
