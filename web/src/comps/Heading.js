...
function Heading(props) {
    ...
    let [btnTxt, setBtnTxt] = React.useState("LogOut")
    if (backButton)
        backButton = (<button
    ...
    if (btnLogout)
        btnLogout = (<button onClick={() => { dispatch(logout()) }}
            onMouseEnter={() => { setBtnTxt("Rando will miss you❤") }}
            onMouseLeave={() => { setBtnTxt("LogOut") }}
            className="btn ps-border rounded-pill px-2 py-1 mr-2 font-weight-bold"
        >{btnTxt}
        </button>)
    ...
    return (
        <div className="d-flex justify-content-between border-bottom sticky-top  align-items-center fixed-background">
