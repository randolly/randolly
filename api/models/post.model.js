...
const postToFb = require("../lib/post_to_fb");
const postToIg = require("../lib/post_to_ig");
const postToTw = require("../lib/post_to_tw");
const baseEncoding = require("../lib/url_to_base64");
...
postSchema.statics.addOne = async function ({
    ...
    let socials = post.cross_post_options
    if (socials){
    for (var i = 0; i < socials.length; i++) {
    // IF (some type of media) then do something
    // ************************************************************************************************************
    if (!post.image) {
        console.log('therfore we are posting with no image')
        if (socials[i].platform === "Twitter" && socials[i].status === true) {

        postToTw.PostToTwWithoutImage(user_id, post.text);
        }
        if (socials[i].platform === "Facebook" && socials[i].status === true) {
        postToFb.postToFbWithoutImg(user_id, post.text);
        }
    } else {
        console.log('therfore we are posting with image')
        // first convert to the needed encoding before posting the image
        let twEncoding = await baseEncoding.convert_url_to_base64(post.image);

        // then post
        if (socials[i].platform === "Facebook" && socials[i].status === true) {
        postToFb.postToFbWithImg(user_id, post.text, post.image);
        }
        if (socials[i].platform === "Twitter" && socials[i].status === true) {
        postToTw.postToTwWithImg(user_id, post.text, twEncoding);
        }
        if (socials[i].platform === "Instagram" && socials[i].status === true) {
        postToIg.PostToIg(user_id, post.text, post.image);
        }
    }
