...
exports.userScheduled = async (req, res, next) => {
    try {
        let username = req.params.username;
        // let page = req.query['p'];
        // page = parseInt(page);
        ...
        let posts = await SCPost.getUserTimeline({ user_id: user._id })
        posts = await serializePosts(posts, req.user)
        user = await serializeUser(user, req.user)
        // console.log(posts)
        res.json({
...
exports.userScheduledOneDay = async (req, res, next) => {
    try {
        let username = req.params.username;
        ...
        let posts = await SCPost.getInSpecificDay({ user_id: user._id, date_without_time: date })
        posts = await serializePosts(posts, req.user)
        user = await serializeUser(user, req.user)
        // console.log(posts)
        res.json({
